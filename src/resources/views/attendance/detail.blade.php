        ...
        <div class="attendance-detail__card">
            ...
            <div class="attendance-detail__group">
                ...
                <div class="attendance-detail__input-area">
                    <span class="attendance-detail__text">{{ Auth::user()->name }}</span>
                </div>
            </div>
            ...
            <div class="attendance-detail__group">
                ...
                <div class="attendance-detail__input-area">
                    <span style="display:none">{{ substr($attendance->clock_in, 0, 5) }}</span>
                    <input type="text" name="clock_in" value="{{ old('clock_in', substr($attendance->clock_in, 0, 5)) }}" ...>
                    <span class="attendance-detail__separator">～</span>
                    <span style="display:none">{{ substr($attendance->clock_out, 0, 5) }}</span>
                    <input type="text" name="clock_out" value="{{ old('clock_out', substr($attendance->clock_out, 0, 5)) }}" ...>
                </div>
                @error('clock_in')
            ...
            <div class="attendance-detail__group">
                ...
                <div class="attendance-detail__input-area">
                    <input type="text" name="rests[{{ $i }}][start]" value="{{ old("rests.$i.start", $rest ? substr($rest->start_time, 0, 5) : '') }}" {{ $isRequested ? 'readonly' : '' }} class="attendance-detail__input">
                    <span class="attendance-detail__separator">～</span>
                    <input type="text" name="rests[{{ $i }}][end]" value="{{ old("rests.$i.end", $rest ? substr($rest->end_time, 0, 5) : '') }}" {{ $isRequested ? 'readonly' : '' }} class="attendance-detail__input">
                </div>
                @error("rests.$i.start")
                    <p class="attendance-detail__error">{{ $message }}</p>
                @enderror
            ...
            <div class="attendance-detail__group">
                ...
                <div class="attendance-detail__input-area">
                    <textarea name="reason" rows="2" {{ $isRequested ? 'readonly' : '' }} class="attendance-detail__textarea">{{ old('reason', $attendance->remark) }}</textarea>
                </div>
                @error('reason')
                    <p class="attendance-detail__error">{{ $message }}</p>
                @enderror
